...
# Hardened lxml parser: disable external-entity resolution + network access so
# a malicious filing cannot exfiltrate data or cause DoS via entity expansion.
...
def _safe_fromstring(xml_bytes: bytes) -> etree._Element:
    return etree.fromstring(xml_bytes, parser=_SAFE_XML_PARSER)
...
_NPORT_FILING_INFO_KEY = ("ACCESSION_NUMBER",)
...
class FormNPORTParser:
        ...
        if doc_count >= 1:
            ...
            if xml_data:
                ...
                result["holdings"] = self._parse_holdings_from_xml(
                    xml_data, result["filing_info"], sec_file_number, accession_number
                )
    ...
    @staticmethod
    def _normalize_for_storage(df: pd.DataFrame, expected_columns: List[str]) -> pd.DataFrame:
        """Align columns + scrub text for CSV safety; reused by both backends."""
    ...
    def save_parsed_data(self, parsed_data: Dict[str, pd.DataFrame]):
        """Persist parsed data via the configured storage backend."""
        ...
        for data_type, df_original in parsed_data.items():
            ...
            if data_type == "holdings":
                df_to_save = self._normalize_for_storage(df_to_save, self._HOLDINGS_COLUMNS)
                self.backend.upsert(
                    "holdings_nport",
                    period_suffix,
                    df_to_save,
                    key_cols=_NPORT_HOLDINGS_KEY,
                    prefer_non_null="CUSIP",
                )

            elif data_type == "filing_info":
                df_to_save = self._normalize_for_storage(df_to_save, self._FILING_INFO_COLUMNS)
                key_cols = (
                    _NPORT_FILING_INFO_KEY
                    if "ACCESSION_NUMBER" in df_to_save.columns
                    else ("CIK", "PERIOD_OF_REPORT", "SEC_FILE_NUMBER")
                )
                self.backend.upsert(
                    "filing_info_nport",
                    period_suffix,
    ...
    def _derive_period_suffix(self, parsed_data: Dict[str, pd.DataFrame], granularity: str = "month") -> str:
        ...
        candidate_columns = [
            ("filing_info", ["PERIOD_OF_REPORT", "FILED_DATE", "REPORT_DATE"]),
            ("holdings", ["PERIOD_OF_REPORT", "FILED_DATE"]),
        ]
    ...
    def _parse_filing_info(self, content: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.warning("NPORT FORMER_COMPANY_NAMES parse failed: %s", e)
        ...
        try:
            ...
            logger.exception("NPORT filing-info post-processing failed: %s", e)
    ...
    def _extract_fund_and_performance_info(self, xml_data: str) -> dict:
        ...
        try:
            root = _safe_fromstring(xml_data.encode("utf-8"))
            ...
            if gen_info is not None:
                ...
                fund_info["FUND_FILE_NUMBER"] = self._get_xml_text(
                    gen_info, "nport:regFileNumber", namespaces
                )
            ...
            if fund_info_elem is not None:
                ...
                fund_info["FUND_TOTAL_ASSETS"] = self._get_xml_text(
                    fund_info_elem, "nport:totAssets", namespaces
                )
                fund_info["FUND_TOTAL_LIABS"] = self._get_xml_text(
                    fund_info_elem, "nport:totLiabs", namespaces
                )
                fund_info["FUND_NET_ASSETS"] = self._get_xml_text(
                    fund_info_elem, "nport:netAssets", namespaces
                )
                fund_info["ASSETS_ATTR_MISC_SEC"] = self._get_xml_text(
                    fund_info_elem, "nport:assetsAttrMiscSec", namespaces
                )
                fund_info["ASSETS_INVESTED"] = self._get_xml_text(
                    fund_info_elem, "nport:assetsInvested", namespaces
                )

                # Payable amounts - within one year
                fund_info["AMT_PAY_ONE_YR_BANKS_BORR"] = self._get_xml_text(
                    fund_info_elem, "nport:amtPayOneYrBanksBorr", namespaces
                )
                fund_info["AMT_PAY_ONE_YR_CTRLD_COMP"] = self._get_xml_text(
                    fund_info_elem, "nport:amtPayOneYrCtrldComp", namespaces
                )
                fund_info["AMT_PAY_ONE_YR_OTH_AFFIL"] = self._get_xml_text(
                    fund_info_elem, "nport:amtPayOneYrOthAffil", namespaces
                )
                fund_info["AMT_PAY_ONE_YR_OTHER"] = self._get_xml_text(
                    fund_info_elem, "nport:amtPayOneYrOther", namespaces
                )

                # Payable amounts - after one year
                fund_info["AMT_PAY_AFT_ONE_YR_BANKS_BORR"] = self._get_xml_text(
                    fund_info_elem, "nport:amtPayAftOneYrBanksBorr", namespaces
                )
                fund_info["AMT_PAY_AFT_ONE_YR_CTRLD_COMP"] = self._get_xml_text(
                    fund_info_elem, "nport:amtPayAftOneYrCtrldComp", namespaces
                )
                fund_info["AMT_PAY_AFT_ONE_YR_OTH_AFFIL"] = self._get_xml_text(
                    fund_info_elem, "nport:amtPayAftOneYrOthAffil", namespaces
                )
                fund_info["AMT_PAY_AFT_ONE_YR_OTHER"] = self._get_xml_text(
                    fund_info_elem, "nport:amtPayAftOneYrOther", namespaces
                )

                # Other financial metrics
                fund_info["DELAY_DELIVERY"] = self._get_xml_text(
                    fund_info_elem, "nport:delayDeliv", namespaces
                )
                fund_info["STANDBY_COMMIT"] = self._get_xml_text(
                    fund_info_elem, "nport:standByCommit", namespaces
                )
                fund_info["LIQUID_PREF"] = self._get_xml_text(fund_info_elem, "nport:liquidPref", namespaces)
                fund_info["CASH_NOT_RPTD_IN_COR_D"] = self._get_xml_text(
                    fund_info_elem, "nport:cshNotRptdInCorD", namespaces
                )
                fund_info["IS_NON_CASH_COLLATERAL"] = self._get_xml_text(
                    fund_info_elem, "nport:isNonCashCollateral", namespaces
                )
            ...
            logger.warning("NPORT: error extracting fund/performance info: %s", e)
    ...
    def _parse_holdings_from_xml(
        ...
        try:
            ...
            filed_date_val = (
                filing_info_df["FILED_DATE"].iloc[0]
                if not filing_info_df.empty and "FILED_DATE" in filing_info_df.columns
                else None
            )
            period_of_report_val = (
                filing_info_df["PERIOD_OF_REPORT"].iloc[0]
                if not filing_info_df.empty and "PERIOD_OF_REPORT" in filing_info_df.columns
                else None
            )
            cik_val = (
                filing_info_df["CIK"].iloc[0]
                if not filing_info_df.empty and "CIK" in filing_info_df.columns
                else None
            )
            ...
            for inv in root.findall(".//nport:invstOrSec", namespaces):
                ...
                if sec_lending is not None:
                    holding["IS_CASH_COLLATERAL"] = self._get_xml_text(
                        sec_lending, "nport:isCashCollateral", namespaces
                    )
                    holding["IS_NON_CASH_COLLATERAL"] = self._get_xml_text(
                        sec_lending, "nport:isNonCashCollateral", namespaces
                    )
                    holding["IS_LOAN_BY_FUND"] = self._get_xml_text(
                        sec_lending, "nport:isLoanByFund", namespaces
                    )
                ...
                if derivative_info is not None:
                    holding["DERIVATIVE_CAT"] = self._get_xml_text(
                        derivative_info, "nport:derivCat", namespaces
                    )
                    holding["COUNTERPARTY_NAME"] = self._get_xml_text(
                        derivative_info, "nport:counterpartyName", namespaces
                    )
                ...
                if not holding["LEI"]:
                    other_id_elem_for_lei = inv.find(
                        ".//nport:idenOther", namespaces
                    )  # Re-find or use existing if safe
            ...
            logger.exception("NPORT holdings parse failed: %s", e)
            return pd.DataFrame()
    ...
    def _convert_holdings_data_types(self, df: pd.DataFrame):
        ...
        numeric_cols = ["BALANCE", "VALUE_USD", "PCT_VALUE", "ANNUAL_RATE", "NUM_PAYMENTS_ARREARS"]
        for col in numeric_cols:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors="coerce")
        ...
        date_cols = [
            ...
            "FILED_DATE",
            "MATURITY_DATE",
        ]  # Added PERIOD_OF_REPORT, FILED_DATE, removed REPORT_DATE
    ...
    def _extract_xml_data(self, content: str) -> Optional[str]:
        ...
        try:
            ...
            nport_match = re.search(
                r"<edgarSubmission[^>]*>.*?</edgarSubmission>", content, re.DOTALL | re.IGNORECASE
            )
            if nport_match:
